...
import supabaseQueryParentCapital from "./supabaseQueryParentCapital"
// 
// @returns an array of landed title objects

export default async function bulkExportTxtFileEBACAVEMARIA(data, selectedCategory, exportFilterInput) {
   ...
   if (selectedCategory === "Provinces") {
      ...
      if (data.length > 0) {
         ...
         data.forEach((entry) => {
            ...
            if (
               ...
               entry.user_province_history.length > 0
            ) {
               ...
               entry.province_history.forEach((historyEntry) => {
                  ...
                  if (historyEntry.culture === null || historyEntry.religion === null || historyEntry.holding_type === null) {
                     entry.county_capital.province_history.forEach((capitalHistoryEntry) => {
                        // If the date entry is 600.1.1 which is the equivalent of no dated entries, write without a date entry
                        if (capitalHistoryEntry.date === "600.1.1") {
                           // Defining province info & concat info the txt string
                           // Culture
                           if (
                              historyEntry.culture === null ||
                              ...
                              const province_culture =
                                 "\n\tculture = " + capitalHistoryEntry.culture;
                              text = text.concat(province_culture);
                           }
                           // Religion
                           if (
                              ...
                              const province_religion =
                                 "\n\treligion = " + capitalHistoryEntry.religion;
                              text = text.concat(province_religion);
                           }
                           // Holding_type
                           if (
                              ...
                              const province_holding_type =
                                 "\n\tholding = " + capitalHistoryEntry.holding_type;
                              text = text.concat(province_holding_type);
                           }

                        } else {
                           // Opening bracket of date entry
                           const date_opening_bracket =
                              "\n\t" + capitalHistoryEntry.date + " = {";
                           text = text.concat(date_opening_bracket);
      ...
      if (data.length > 0) {
         // console.log(data[0]);
         // console.log(data[0].title_history_duplicate);

         // Looping over all the history entries info
         data.forEach((entry) => {
            // Checking there is something inside the entry
            if (
               entry.title_history_duplicate.length > 0
               // entry.user_title_history.length > 0
            ) {
               // console.log(entry.title_history_duplicate);
               ...
               // Looping over the vanilla date entries inside the title entry
               entry.title_history_duplicate.forEach((historyEntry) => {
                  // Opening bracket of date entry
                  const date_opening_bracket =
                     "\n\t" + historyEntry.date + " = {";
                  text = text.concat(date_opening_bracket);
                  ...
                  //    entry.county.title_history_duplicate.forEach((historyEntry) => {
                  //       // Opening bracket of date entry
                  //       const date_opening_bracket =
                  //          "\n\t" + historyEntry.date + " = {";
                  //       text = text.concat(date_opening_bracket);
            ...
            if (entry.title_history_duplicate.length <= 0 && entry.province !== null) {
               // Opening bracket of title name entry
               const title_name_opening_bracket = entry.county + " = {";
               // Writing to the txt string
               text = text.concat(title_name_opening_bracket);

               // Parent county history entries
               entry.province.county.title_history.forEach((historyEntry) => {

                  // If there is nothing, write nothing
                  if (
                     historyEntry.liege === null &&
                     historyEntry.liege === "undefined" &&
                     historyEntry.liege === "" &&
